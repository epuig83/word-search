...
- **i18n invariant:** the three language blocks in `i18n.js` must have the **same set of keys** (currently 178 each for `es`/`ca`/`en`). Verify with:
  ```bash
  grep -cE '^\s+[a-z_]+:\s' i18n.js   # 534 total = 178 × 3
  ```
