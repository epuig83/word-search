...
- **i18n invariant:** the three language blocks in `i18n.js` must have the **same set of keys** (currently 181 each for `es`/`ca`/`en`). Verify with:
  ```bash
  grep -cE '^\s+[a-z_]+:\s' i18n.js   # 543 total = 181 × 3
  ```
