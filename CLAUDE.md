...
- **i18n invariant:** the three language blocks in `i18n.js` must have the **same set of keys** (currently 170 each for `es`/`ca`/`en`). Verify with:
  ```bash
  grep -cE '^\s+[a-z_]+:\s' i18n.js   # 510 total = 170 × 3
  ```
